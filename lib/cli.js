...
cli.responders.help = function() {
	var commands = {
		...
		'more user info' : 'Show details of a specific user --(user info)', 
...
cli.responders.moreUserInfo = function(str) {
	...
	if(userId) {
		_data.read('users', userId, function(err, userData) {
			if(!err && userData) {
				//remove hash password from userData
		...
		if(!err && checkIds && checkIds.length > 0) {
			...
			checkIds.forEach(function(checkId) {
				_data.read('checks', checkId, function(err, checkData) {
					...
					var userString = str.toLowerCase()
					...
					// if user has specified state, or has not specified any state include current check accordingly
					if(userString.indexOf('--'+state) > -1 || (userString.indexOf('--down') == -1 && userString.indexOf('--up') == -1)) {
...
cli.processInput = function(str) {
	...
	if(str) {
		...
		uniqueInputs.some(function(input) {
			// if one of the inputs is a substring of the string the user entered
