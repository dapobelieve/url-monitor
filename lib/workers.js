...
workers.alertUser = function(checkData) {
	...
	helpers.sendTwilioSms(checkData.phone, message, function(err) {
		if(!err) {
			...
			debug("Error sending sms to user: "+checkData.phone)
		}
	})
}
...
workers.processCheckOutcome = function(originalCheckData, checkOutcome) {
	...
	_data.update('checks', newCheckData.id, newCheckData, function(err) {
		if(!err) {
			...
			debug("Error trying to save updates to check: "+originalCheckData.id)
		}
	})
}
...
workers.validateCheckData = function(checkData) {
	debug(typeof(checkData.userPhone) == "number")
...
workers.loop = function() {
	...
	}, 1000 * 60) // run every 6 minutes
}

workers.rotateLogs = function() {
	// list all none compressed logged files
	_logs.list(false, function(err, logs) {
		if(!err && logs && logs.length > 0) {
			logs.forEach(function(logFile) {
				// Compress the data to different file
				var logId = logFile.replace('.log', '');
				var newFileId = `${logId}-${new Date().getFullYear()}-${new Date().getMonth()}-${new Date().getDate()}-${new Date().getMinutes()}-${new Date().getSeconds()}`
...
// timer to execute logs-rotation once per day
