...
handlers.index = function(data, callback) {
	if(data.method == 'get') {
		var templateData = {
			'index.title': "This is the title index set from the page",
			'page.title': "HomePage 🏡"
		}
...
/**
 * Public Folder to serve assets
 */
handlers.public = function(data, callback) {
	if(data.method == 'get') {
		// get filename being requested
		var trimmedAsset = data.trimmedPath.replace('public/', '').trim();

		if(trimmedAsset.length > 0) {
			helpers.getStaticAsset(trimmedAsset, function(err, data) {
				if(!err && data) {
